...
class Receiver:
    ...
    def start(self, request):
        self.stop()
        if self.demo: LOG.info("Demonstração: recepção simulada em %s", request.frequency_hz); return
    ...
    def stop(self):
        ...
        self.processes = []
    @property
    def running(self): return self.demo or bool(self.processes) and all(p.poll() is None for p in self.processes)
